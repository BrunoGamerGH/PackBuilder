package me.bruno.packbuilder.apache;
...
public class FilenameUtils {
        ...
        if (filename == null) {
            ...
            if (len == 0) {
                ...
                if (ch0 == ':') {
                    ...
                    int posUnix;
                    if (ch0 == '~') {
                        int posUnix1 = filename.indexOf(47, 1);
                        posUnix = filename.indexOf(92, 1);
                        if (posUnix == -1 && posUnix1 == -1) {
                            return len + 1;
                        } else {
                            posUnix = posUnix1 == -1 ? posUnix1 : posUnix;
                            posUnix1 = posUnix == -1 ? posUnix : posUnix1;
                            return Math.min(posUnix, posUnix1) + 1;
                        }
                    } else {
    ...
    public static boolean wildcardMatch(String filename, String wildcardMatcher, IOCase caseSensitivity) {
        if (filename == null && wildcardMatcher == null) {
            ...
            do {
                if (backtrack.size() > 0) {
                    int[] array = (int[])((int[])backtrack.pop());
                    wcsIdx = array[0];
                    textIdx = array[1];
                    anyChars = true;
                }
